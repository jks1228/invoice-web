...
- **관리자 접근 제어 (A005)**: 인증·권한 — **이번 범위에서 제외**, Task 018에서 결정 사항으로 관리 (아래 "범위 결정 사항" 참조)
...
> **결정: 관리자 인증(A005)은 이번 고도화(Phase 5~7) 범위에 포함하지 않는다.**
>
> - 이번 범위는 사용자가 지정한 **"관리자 대시보드 기초" = 목록 조회 + 링크 복사 + 다크모드 확장**까지로 한정한다. 임의 확장하지 않는다.
> - **⚠️ 보안 위험 명시**: Task 012 이후 **관리자 라우트(`/admin`)는 인증 없이 URL을 아는 누구나 접근 가능**하다. 이 상태에서는 **전체 클라이언트 목록·금액·상태가 공개 노출**되므로, 공개 인보이스 페이지(링크를 아는 사람만 1건 확인)보다 노출 범위가 훨씬 넓다.
> - **임시 완화책(Task 012에 포함)**: 관리자 라우트에 `robots: { index: false, follow: false }` 메타데이터를 적용해 검색 노출을 차단한다. 이는 완화책일 뿐 접근 제어가 아니다.
> - **프로덕션 배포 게이트**: 인증(Task 018)이 완료되기 전에는 **관리자 영역을 공개 도메인에 배포하지 않는다**. 로컬/프리뷰 환경에서만 사용한다 (Task 020 배포 체크리스트에 게이트로 명시).
> - 인증은 **Task 018**로 분리해 두되 상태는 "보류 — 결정 필요"로 유지한다. 착수 시 방식(단일 비밀번호 + 서명 쿠키 / Basic Auth / Vercel 인증 / 외부 IdP)을 먼저 확정한다.
...
- **Task 018: 관리자 접근 제어(인증) 구현 (A005)** - 보류 (결정 필요)
  - **현재 상태: 이번 고도화 범위에서 제외됨.** 위 "🔒 범위 결정 사항" 참조. Task 012~017 완료 시점의 `/admin`은 **인증 없이 누구나 접근 가능(보안 위험)** 하며, 이 Task가 완료되기 전에는 공개 도메인에 배포하지 않는다
  - 착수 전 결정할 항목:
    - 인증 방식: (a) 단일 관리자 비밀번호 + 서명된 HttpOnly 쿠키, (b) Vercel 배포 보호(Password Protection), (c) Basic Auth를 `proxy.ts`에서 처리, (d) 외부 IdP(NextAuth 등)
    - 사용자 수(1인 발행자 vs 다중 계정), 세션 유지 기간, 로그아웃 요구 여부
  - 예상 구현 범위 (방식 (a) 기준):
    - `src/app/admin/login/page.tsx` + Server Action 로그인 처리 (React Hook Form + Zod)
    - `ADMIN_PASSWORD_HASH`·`ADMIN_SESSION_SECRET` 환경 변수 추가 (`src/lib/env.ts` 스키마 확장, 서버 전용 — `NEXT_PUBLIC_` 금지)
    - `proxy.ts`(Next.js 16 방식) 또는 관리자 `layout.tsx`에서 세션 쿠키 검증 → 미인증 시 로그인으로 리다이렉트
    - 로그인 실패 rate limit(무차별 대입 방어), 세션 만료·로그아웃 처리
    - Task 012에서 넣은 "인증 미적용" 개발용 안내 배너 제거
  - **테스트** (Playwright MCP):
    - 정상: 올바른 비밀번호 로그인 → `/admin` 접근 및 목록 표시, 새로고침 후 세션 유지
    - 정상: 로그아웃 → 보호 라우트 재접근 시 로그인으로 리다이렉트
    - 오류: 잘못된 비밀번호 → 사용자 친화적 에러, 비밀번호/해시 값 화면·네트워크 응답 미노출
    - 오류: 미인증 상태로 `/admin` 직접 접근 → 로그인 리다이렉트(목록 데이터가 잠깐이라도 렌더링되지 않을 것)
    - 엣지: 만료/위조된 세션 쿠키 → 거부 후 재로그인 유도
    - 엣지: 로그인 실패 연속 시도 → rate limit 동작, 공개 페이지(`/`·`/invoice/{id}`·PDF)는 영향 없이 계속 공개 접근 가능
    - 반응형/품질: 로그인 화면 전 뷰포트·다크모드 정상, 콘솔 에러 0건
