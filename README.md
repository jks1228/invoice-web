...
**PDF로 다운로드**할 수 있는 공개 인보이스 플랫폼입니다.

- **발행자**: 프리랜서 / 소상공인 — Notion 데이터베이스에서 견적서를 관리
...
│   ├── invoice.ts                    # 화면·PDF 공용 유틸 (소계 계산, 상태 라벨 등)
│   ├── format.ts                     # 통화·날짜 포맷
│   ├── notion/                       # Notion API 클라이언트 + 데이터 매핑
│   └── pdf/                          # @react-pdf/renderer 문서 정의
└── types/                            # Invoice 등 도메인 타입
...
- [`docs/guides/deployment.md`](docs/guides/deployment.md) — Vercel 배포 가이드
